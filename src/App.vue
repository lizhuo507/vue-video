  ...
  <div class="app-main">
    <div class="app-container">
      ...
      <div class="search-container">
        <el-form ref="queryFormRef" :model="queryParams" :inline="true">
          ...
          <el-form-item prop="keywords" label="设置阈值(%)">
            <el-input-number v-model="threshold" :min="0" :max="100" style="width: 80px" controls-position="right" />
          </el-form-item>
          <el-form-item prop="keywords" label="统计依据">
            <el-select v-model="queryParams.countType" placeholder="" style="width: 180px"
              @change="() => handleQuery()">
          ...
          <el-form-item>
            ...
            <el-button @click="handleExport" type="success" :disabled="!ids.length" :icon="Download">导出</el-button>
          </el-form-item>
        </el-form>
      </div>
      ...
      <el-card shadow="never" class="table-container">
        <template #header>
          <div class="flex items-center">
            ...
            <el-radio-group v-model="time">
              ...
              <el-radio-button label="自定义" :value="4" @click="dialogVisible2 = true" />
            </el-radio-group>
          </div>
        </template>

        <el-table ref="dataTableRef" v-loading="loading" :data="tableData" highlight-current-row border
          ...
          <el-table-column v-if="queryParams.countType == 1" label="管养单位" prop="road_sec" width="400" />
          <el-table-column v-else label="路段名称" prop="install_place" width="400" />
          <el-table-column label="接入数" prop="collect_num">
          </el-table-column>

          <el-table-column label="在线数" prop="online_num" />
          <el-table-column label="离线数" prop="outline_num" />
          <el-table-column :label="time !== 7 && time !== 30 ? '实时在线率' : '平均在线率'" prop="online_rate">
            <template #default="{ row }">
              <span :class="{ 'text-red-500': Number(row.online_rate.slice(0, -1)) < threshold }">{{ row.online_rate
                }}</span>
            </template>
          </el-table-column>

          <el-table-column fixed="right" label="操作" width="150" v-if="time !== 7 && time !== 30">
            <template #default="{ row }">
        ...
        <el-pagination class="mt-5 flex justify-end" v-model:current-page="queryParams.page"
          v-model:page-size="queryParams.pageSize" background layout="total, sizes, prev, pager, next, jumper"
          :page-sizes="[10, 20, 50, 100]" :total="total" @size-change="handleSizeChange"
          @current-change="handleCurrentChange" />
      </el-card>

      <!-- 详情弹窗 -->
      <el-dialog v-model="dialogVisible" title="详情" width="65%" @close="closeDialog">
        <el-table v-loading="dialogLoading" :data="list.slice((currentPage - 1) * 10, currentPage * 10)"
          style="width: 100%">
          <template #empty>
            <el-empty />
          </template>
          ...
          <el-table-column prop="online_status" label="在线状态" width="100">
            <template #default="{ row }">
              <el-tag v-if="row.online_status === '在线'" type="success">在线</el-tag>
              <el-tag v-else type="info">离线</el-tag>
            </template>
          </el-table-column>
        </el-table>
        <el-pagination class="flex justify-end" layout="prev, pager, next" :total="list.length" :page-size="10"
      ...
      <!-- 自定义弹框 -->
      <el-dialog v-model="dialogVisible2" title="自定义日期" width="450px" @close="closeDialog">

        <el-date-picker v-model="date" type="date" placeholder="请选日期" value-format="YYYY-MM-DD" />
        <template #footer>
          <div class="dialog-footer">
      ...
      <!-- 下载进度条 -->
      <el-dialog title="正在导出，请耐心等待" v-model="fileDown.loadDialogStatus" :close-on-click-modal="false"
        :close-on-press-escape="false" :show-close="true" width="20%" @close="source.cancel('下载被取消')">
...
console.log('window.$wujie?.props: ', window.$wujie?.props.roleInfo.user?.fullName);
// 监听时间区间
watch(
  time,
  (newValue) => {
    ...
    if (newValue == 1) { //昨日
      queryParams.start = dayjs().subtract(1, 'day').startOf('day').format(format)
      queryParams.end = dayjs().subtract(1, 'day').endOf('day').format(format)
    }
    if (newValue == 7) { //近七天
      queryParams.start = dayjs().subtract(7, 'day').startOf('day').format(format)
      queryParams.end = dayjs().subtract(1, 'day').endOf('day').format(format)
    }
    if (newValue == 30) { //近一月
...
async function openDialog(row) {
  ...
  const res: any = await request.post(`/vhcioiset/videoPointDtList?token=${token}`, { ...params, onFlag: time.value == 0 ? "on" : "off" }).finally(() => {
