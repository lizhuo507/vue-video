      ...
      >
        ...
        >
          ...
          <el-table-column
            ...
            label="网关编号"
          />
          <el-table-column prop="server_name" label="网关名称"  width="160"/>
...
const videoQuality = ref(route.meta.streamType as string || "1"); // 默认使用标清
const systemType = ref(route.meta.systemType as string || "highway"); // 默认为高速系统
...
const videoRequest = async (row: any): Promise<any> => {
  const res: any = await request.post(
    `/vhcioiset/videoPointUrl?token=${token}`,
    {
      ...
      streamType: videoQuality.value, // 使用路由传来的视频质量参数
    }
  );
